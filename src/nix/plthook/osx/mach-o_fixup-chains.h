...
struct dyld_chained_starts_in_image {
    uint32_t seg_count;
    uint32_t seg_info_offset[1]; // each entry is offset into this struct for
                                 // that segment
...
struct dyld_chained_starts_in_segment {
    ...
    uint16_t page_start[1];     // each entry is offset in each page of first
                            // element in chain or DYLD_CHAINED_PTR_START_NONE
                            // if no fixups on page
    // uint16_t    chain_starts[1];    // some 32-bit formats may require
    // multiple starts per page. for those, if high bit is set in page_starts[],
    // then it is index into chain_starts[] which is a list of starts the last
    // of which has the high bit set
};

enum {
    DYLD_CHAINED_PTR_START_NONE =
        0xFFFF, // used in page_start[] to denote a page with no fixups
    DYLD_CHAINED_PTR_START_MULTI = 0x8000, // used in page_start[] to denote a
                                           // page which has multiple starts
...
struct dyld_chained_ptr_64_rebase {
    uint64_t target : 36, // 64GB max image size (DYLD_CHAINED_PTR_64 => vmAddr,
                          // DYLD_CHAINED_PTR_64_OFFSET => runtimeOffset)
        high8 : 8, // top 8 bits set to this (DYLD_CHAINED_PTR_64 => after slide
                   // added, DYLD_CHAINED_PTR_64_OFFSET => before slide added)
...
struct dyld_chained_ptr_64_kernel_cache_rebase {
    uint64_t target : 30, // basePointers[cacheLevel] + target
        cacheLevel : 2, // what level of cache to bind to (indexes a mach_header
                        // array)
        diversity : 16, addrDiv : 1, key : 2,
...
struct dyld_chained_import_addend64 {
    uint64_t lib_ordinal : 16, // -15 .. 65520 (0xFFF1 .. 0xFFF0)
        weak_import : 1, reserved : 15, name_offset : 32;
    uint64_t addend;
};
